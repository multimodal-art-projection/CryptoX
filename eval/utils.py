...
def parse_init():
    ...
    # 添加命令行参数
    parser.add_argument('-i', '--input', type=str, default="data_construct/crypto_data", help='要进行评估的数据文件地址或者文件目录')
    parser.add_argument('-r', '--recursive', action='store_true', help='如果需要对一个目录下的所有jsonl文件进行评估，需要加上-r指令，并指定输入文件夹')
